 ...
 */
public class HistoryEndpointTest extends AbstractEnpointTest {
  public HistoryEndpointTest(String token) {
    super(token);
  }

  @Override
  public void setupTest() {
    request = new ApiRequest("http://localhost:8901/system/history", ApiRequest.Method.GET, authToken);
    validator = new AbstractBodyApiResponseValidator("application/json", HttpStatus.SC_OK) {
      ...
      public void validateBody(HttpResponse response, String responseBody) throws Exception {
        ...
        try {
          ...
          history = gson.fromJson(responseBody, new TypeToken<List<HistoryEntry>>() {}.getType());
        } catch(Exception e) {
