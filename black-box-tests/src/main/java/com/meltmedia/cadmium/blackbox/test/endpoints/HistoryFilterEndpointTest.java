 ...
 */
public class HistoryFilterEndpointTest extends AbstractEnpointTest {
  public HistoryFilterEndpointTest(String token) {
    super(token);
  }

  @Override
  public void setupTest() {
    request = new ApiRequest("http://localhost:8901/system/history?filter=true", ApiRequest.Method.GET, authToken);
    validator = new AbstractBodyApiResponseValidator("application/json", HttpStatus.SC_OK) {
      ...
      public void validateBody(HttpResponse response, String responseBody) throws Exception {
        ...
        assertTrue("No history entries returned", history.size() > 0);
        ...
        assertTrue("Filter parameter did not work as expected.", !hasBad);
      }
    };
  }
}
